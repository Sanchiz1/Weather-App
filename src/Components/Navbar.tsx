...
import { RequestCity, RequestLocation, getCookie, setCookie } from '../Api/Requests';
...
function AppNavbar() {
  ...
  return (
    <Navbar expand="lg" className="bg-body-tertiary shadow">
      <Container fluid>
        <Navbar.Brand className='text-customlightblue disabled'>YourWeather</Navbar.Brand>
          ...
          <Dropdown className='me-2'>
            ...
            <Dropdown.Menu>
              <div className='d-flex p-2'>
                ...
                <Form.Check type='switch' className='ms-auto' checked={theme === "dark"} onClick={() => { SwitchThemes(); setTheme(theme == "light" ? "dark" : "light") }}></Form.Check>
              </div>
            </Dropdown.Menu>
          </Dropdown>
          <Form className="d-flex me-2" onSubmit={e => { e.preventDefault(); searchCity() }}>
            <InputGroup>
              <Form.Control
                type="search"
              ...
              <Dropdown.Menu show={showPredictions && cityHint !== ''} className='p-0'>
                <Button className='p-2 m-0 w-100' variant='outline' type='submit' onClick={() => { setSearch(cityHint) }}>{cityHint}</Button>
              </Dropdown.Menu>
              <Button variant="outline-customlightblue shadow-sm" type='submit'>Search</Button>
            </InputGroup>
          </Form>
        </Navbar.Collapse>
      </Container>
