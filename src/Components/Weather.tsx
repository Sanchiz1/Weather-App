                    ...
                    <Card.Body className='py-1 px-3'>
                        <Row className='mb-2'>
                            {weather.days.map((e, index) => {
                                ...
                                return (
                                    <Col className='m-0 p-0' key={index} onClick={() => { setHour(0); setDay(index); }}>
                                        <Card className={(index === day) ? 'text-white bg-customblue p-0 customshadow' : 'text-white bg-customlightblue p-0 customhover customshadow'}>
                                            <Card.Body className='p-1'>
                                                <p className='m-0 text-center'>{Math.round(temp[1])}{'\u00b0'}</p>
                                                <div className='d-flex justify-content-center'>
                                                    <p className='m-0 w-25 text-center'><WeatherIcon id={e.day[Math.round((e.day.length - 1) / 2)].weather[0].icon}></WeatherIcon></p>
                                                </div>
                                                <p className='m-0 text-center'>{Math.round(temp[0])}{'\u00b0'}</p>
                                                <p className='m-0 text-center'>{getShortDateSting(toDateTime(e.day[0]?.dt + city.timezone + new Date().getTimezoneOffset() * 60))}</p>
                                            </Card.Body>
                                        </Card>
                                    </Col>)
                            })}
                                ...
                                <Card.Body>
                                    <Row className='Row'>
                                        <Col className='mb-3'>
                                            ...
                                            <h3 className='m-0'>{getDateSting(toDateTime(weather.days[day].day[hour]?.dt + city.timezone + new Date().getTimezoneOffset() * 60))}</h3>
                                            <h3 className='m-0'>{getTimeSting(toDateTime(weather.days[day].day[hour]?.dt + city.timezone + new Date().getTimezoneOffset() * 60))}</h3>
                                        </Col>
                                        <Col className='mb-3'>
                                            <Row>
                                                <Col className='d-flex justify-content-end'>
                                                    <div className='w-75'><WeatherIcon id={weather.days[day].day[hour]?.weather[0].icon}></WeatherIcon></div>
                                                </Col>
                                                <Col className='d-flex justify-content-start flex-column p-0'>
                                                    <h2 className='m-0'>{Math.round(weather.days[day].day[hour]?.main.temp)}{'\u00b0'}</h2>
                        ...
                        <Row>
                            {Array.from({ length: 8 - weather.days[day]?.day.length > 3 ? 8 - weather.days[day]?.day.length - 4 : 8 - weather.days[day]?.day.length }, (_, index) =>
                            ...
                            {weather.days[day]?.day.map((e, index) =>
                                <Card key={index} className={(index === hour) ? 'text-white bg-customblue rounded-0 w-25 p-0 customshadow' : 'text-white bg-customlightblue rounded-0 w-25 p-0 customhover customshadow'}
