...
const id = "1a5ac5a7023db30b858ffab987225a92"
...
export function RequestWeather(city: string): Observable<any> {
    ...
    const apiCall = fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${id}&units=metric`)
...
export function RequestCity(search: String) {
    ...
    const apiCall = fetch(`https://api.api-ninjas.com/v1/city?name=${search}`, {
        method: 'GET',
        headers: {
            'X-Api-Key': '1buNBQu0pr+lXWtswrKxrA==EDpkWzjqHmCYzgdW',
        },
    })
        .then(response => response.json())
        .then((responseJson) => {
